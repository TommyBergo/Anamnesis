...
/** End-to-end evaluation of the real, on-device medical RAG pipeline ([ClinicalPdfProcessor] → embedder → [MedicalVectorRepository] → [MedicalRagEngine]) against the rule-based single-document QA set. */
@RunWith(AndroidJUnit4::class)
class MedicalRagEvaluationTest {

    companion object {
        private const val TAG = "MedicalRagEvaluationTest"
        ...
        private const val TEST_DB_NAME = "objectbox-rag-evaluation-test"
    ...
    fun setUp() {
        ...
        Log.i(TAG, "setUp: building ObjectBox store")
        boxStore = MyObjectBox.builder()
            .androidContext(targetContext.applicationContext)
            .name(TEST_DB_NAME)
            .build()
        Log.i(TAG, "setUp: ObjectBox store ready")
        ...
        Log.i(TAG, "setUp: ClinicalPdfProcessor ready")
    ...
    private fun loadReaderModule(args: Bundle): LlmModule? {
        ...
        if (!File(modelPath).exists() || !File(tokenizerPath).exists() ||
            ...
            Log.w(
                TAG,
                "Reader model not found at $modelPath / $tokenizerPath" +
                    (if (dataPaths.isNotEmpty()) " / ${dataPaths.joinToString(", ")}" else "") +
                    " - skipping generation evaluation (retrieval metrics are unaffected). Push a " +
                    "model first (see this class's doc comment) or pass " +
    ...
    private fun parseItem(line: String): BenchmarkItem {
        ...
        return BenchmarkItem(
            ...
            pageNumber = obj.getInt("page_number"),
...
""".trimStart()
    ...
    @Test
    fun runMedicalRagEvaluation() {
        val targetContext = InstrumentationRegistry.getInstrumentation().targetContext
        val testContext = InstrumentationRegistry.getInstrumentation().context

        val maxItems = InstrumentationRegistry.getArguments()
            .getString("maxItems", "")
            ?.toIntOrNull()

        val startIndex = InstrumentationRegistry.getArguments()
            .getString("startIndex", "")
            ?.toIntOrNull() ?: 0

        val chunkSize = InstrumentationRegistry.getArguments()
            .getString("chunkSize", "")
            ?.toIntOrNull() ?: 500

        val overlapChars = InstrumentationRegistry.getArguments()
            .getString("overlapChars", "")
            ?.toIntOrNull() ?: 100
        ...
        Log.i(TAG, "Loaded ${items.size} benchmark items from $datasetAsset" + (if (startIndex > 0) " (starting at index $startIndex)" else "") + (maxItems?.let { " (capped at $it via maxItems)" } ?: "") + " (chunkSize=$chunkSize, overlapChars=$overlapChars)")
        if (readerModule == null) {
            Log.i(TAG, "No reader model loaded - generation metrics will be omitted")
        }

        val ingestedPatients = mutableSetOf<String>()
        for ((index, item) in items.withIndex()) {
            if (!ingestedPatients.add(item.patientId)) {
                Log.i(TAG, "[${index + 1}/${items.size}] Skipped patient ${item.patientId}: already ingested")
                continue
            }
            val patientPrefix = "Patient_${item.patientId}_"
            val patientDocs = (testContext.assets.list(pdfAssetDir) ?: emptyArray())
                .filter { it.startsWith(patientPrefix) }
            if (patientDocs.isEmpty()) {
                Log.w(TAG, "[${index + 1}/${items.size}] No admission PDFs found for patient ${item.patientId} (prefix $patientPrefix) - falling back to ${item.documentName}")
            }
            val docsToIngest = patientDocs.ifEmpty { listOf(item.documentName) }
            for (documentName in docsToIngest) {
                ...
                val admissionId = ADMISSION_ID_REGEX.find(documentName)?.groupValues?.get(1)
                    ?: documentName
                ...
                pdfFile.delete()
                Log.i(TAG, "[${index + 1}/${items.size}] Ingested $documentName (patient ${item.patientId}): ${chunks.size} chunks")
            }
        }
        ...
        for ((index, item) in items.withIndex()) {
            ...
            val topChunks = retrieved.take(GENERATION_TOP_K).map { it.chunk.content.take(GENERATION_CONTEXT_CHAR_CAP) }
            ...
            if (module != null) {
                module.resetContext()
                val evalPrompt = buildFactualQaPrompt(item.question, topChunks)
        ...
        writeResultsJson(results, n, hitRateAt1, hitRateAt3, hitRateAt5, meanPrecisionAt3, meanPrecisionAt5, meanPrecisionAt10, meanNdcgAt3, meanNdcgAt10, mrr, map, avgSemanticSimilarity, generationPassRate)
    ...
    private fun writeResultsJson(
        ...
        val outFileName = "medical_rag_evaluation_results.json"
        ...
        for (r in results) {
            ...
            itemJson.put("section", r.section)
            itemJson.put("question_type", r.questionType)
            itemJson.put("hit_at_1", r.hitAt1)
            itemJson.put("hit_at_3", r.hitAt3)
            itemJson.put("hit_at_5", r.hitAt5)
            itemJson.put("precision_at_3", r.precisionAt3)
            itemJson.put("precision_at_5", r.precisionAt5)
            itemJson.put("precision_at_10", r.precisionAt10)
            itemJson.put("ndcg_at_3", r.ndcgAt3)
            itemJson.put("ndcg_at_10", r.ndcgAt10)
            itemJson.put("reciprocal_rank", r.reciprocalRank)
            itemJson.put("average_precision", r.averagePrecision)
            itemJson.put("top_chunk_preview", r.topChunkPreview)
