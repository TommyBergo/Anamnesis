...
/** Multi-document evaluation of [MedicalRagEngine] (Table 3's dense-retriever rows) against `QA/multidoc_dataset_llm_main_same_patient.jsonl`, retrieving per distinct patientId and generating from rank-interleaved chunks across patients. */
@RunWith(AndroidJUnit4::class)
class MultiDocRagEvaluationTest {

    companion object {
        private const val TAG = "MultiDocRagEvalTest"
        ...
        private const val TEST_DB_NAME = "objectbox-rag-multidoc-evaluation-test"

        private const val RETRIEVAL_TOP_K = 10
        private const val WRONG_ADMISSION_TOP_K = 3
        private const val GENERATION_CONTEXT_CHAR_CAP = 500
    ...
    data class ItemResult(
        ...
        val question: String,
        val topChunks: List<String>
    )

    private lateinit var boxStore: BoxStore
    private lateinit var vectorRepository: MedicalVectorRepository
    private lateinit var embedder: Embedder
    private lateinit var pdfProcessor: ClinicalPdfProcessor
    private lateinit var ragEngine: MedicalRagEngine
    ...
    fun setUp() {
        ...
        Log.i(TAG, "setUp: deleting any stale ObjectBox files for $TEST_DB_NAME")
        ...
        ragEngine = MedicalRagEngine(vectorRepository, embedder)
        ...
        if (!File(modelPath).exists() || !File(tokenizerPath).exists() || dataPaths.any { !File(it).exists() }) {
            Log.w(
                ...
                "Reader model not found at $modelPath / $tokenizerPath - skipping generation evaluation " +
                    "(retrieval metrics are unaffected). Push a model first or pass " +
                    "-Pandroid.testInstrumentationRunnerArguments.skipGeneration=true to silence this."
            )
            return null
        }
...
""".trimStart()
    ...
    private fun packGenerationContexts(
        ...
        perPatientRetrieved: Map<String, List<ScoredChunk>>,
        answerReserve: Int,
        charsPerToken: Double,
        contextCharCap: Int
    ): Pair<List<String>, Int> {
        val candidatesByPatient = patientIds.associateWith { pid ->
            perPatientRetrieved[pid].orEmpty()
                .take(GENERATION_CANDIDATE_TOP_K_PER_PATIENT)
                .map { scored ->
                    val label = "[Patient $pid, admission ${scored.chunk.documentId}]: "
                    label + scored.chunk.content.take(contextCharCap)
                }
        }
        val totalCandidates = candidatesByPatient.values.sumOf { it.size }

        val ordered = mutableListOf<String>()
        for (rank in 0 until GENERATION_CANDIDATE_TOP_K_PER_PATIENT) {
            for (pid in patientIds) {
                candidatesByPatient[pid]?.getOrNull(rank)?.let { ordered.add(it) }
            }
        }
        ...
        for (block in ordered) {
            val fullCandidate = admitted + block
            val fullPrompt = buildFactualQaPrompt(question, fullCandidate)
            if (estimateTokens(fullPrompt, charsPerToken) <= promptTokenBudget) {
                admitted.add(block)
                continue
            }

            val currentPromptTokens = estimateTokens(
                buildFactualQaPrompt(question, admitted), charsPerToken
            )
            ...
            val charsAvailable = ((remainingTokens - labelTokens) * charsPerToken).toInt()
                .coerceAtMost(content.length)

            if (charsAvailable >= MIN_PARTIAL_CONTEXT_CHARS) {
                val partial = label + content.take(charsAvailable)
                val partialPrompt = buildFactualQaPrompt(question, admitted + partial)
                if (estimateTokens(partialPrompt, charsPerToken) <= promptTokenBudget) {
    ...
    private fun parseStatInt(statsText: String?, key: String): Int? {
        ...
        for (alias in aliases) {
            val pattern = Regex("(?i)" + Regex.escape(alias) + "\\s*[:=]\\s*(\\d+)")
            val value = pattern.find(statsText)?.groupValues?.getOrNull(1)?.toIntOrNull()
    ...
    private fun generateOnce(
        ...
        val promptTokens = parseStatInt(statsJson, "prompt_tokens")
        val generatedTokens = parseStatInt(statsJson, "generated_tokens")
        ...
        return GenerationAttempt(
            ...
            sawChatmlStopToken = sawChatmlStopToken,
            statsJson = statsJson,
            error = error
        )
    }
    ...
    fun runMultiDocRagEvaluation() {
        ...
        for ((index, item) in items.withIndex()) {
            ...
            val perPatientRetrieved: Map<String, List<ScoredChunk>> = patientIds.associateWith { pid ->
                runBlocking {
            ...
            val topChunks = patientIds.flatMap { pid ->
                perPatientRetrieved[pid].orEmpty().take(3).map {
                    "[Patient $pid, admission ${it.chunk.documentId}]: ${it.chunk.content}"
                }
            }
            ...
            if (module != null) {
                val budgets = listOf(
                    GenerationBudget(answerReserve = 384, charsPerToken = 2.0, contextCharCap = 500),
                    GenerationBudget(answerReserve = 640, charsPerToken = 1.8, contextCharCap = 320),
                    GenerationBudget(answerReserve = 896, charsPerToken = 1.6, contextCharCap = 220),
                    GenerationBudget(answerReserve = 1152, charsPerToken = 1.4, contextCharCap = 160)
                )
                ...
                for ((attemptIndex, budget) in budgets.withIndex()) {
                    val (contexts, droppedBlocks) = packGenerationContexts(
                        ...
                        patientIds,
                        perPatientRetrieved,
                        budget.answerReserve,
                        budget.charsPerToken,
                        budget.contextCharCap
                    )
                    val evalPrompt = buildFactualQaPrompt(item.question, contexts)
                    val estimatedPromptTokens = estimateTokens(evalPrompt, budget.charsPerToken)
                    ...
                    Log.i(
                        ...
                        "[${item.id}] generationInput attempt=${attemptIndex + 1}/${budgets.size} " +
                            ...
                            "estPromptTokens=$estimatedPromptTokens blocks=${contexts.size} dropped=$droppedBlocks"
                    )

                    val attempt = generateOnce(
                        module, evalPrompt, contexts.size, droppedBlocks, item.id
                    )
                    attemptsUsed++
                    finalAttempt = attempt

                    val clean = attempt.answer.isNotBlank() &&
                        attempt.error == null && !attempt.likelyTruncated
                    Log.i(
                        TAG,
                ...
                val attempt = finalAttempt
                    ?: throw IllegalStateException("[${item.id}] no generation attempt was executed")
                val finalClean = attempt.answer.isNotBlank() &&
                    attempt.error == null && !attempt.likelyTruncated
                ...
                if (!finalClean) {
                    throw IllegalStateException(
                        "[${item.id}] reader generation remained invalid after $attemptsUsed attempts: " +
                            ...
                            "generatedTokens=${attempt.generatedTokens} blocks=${attempt.contextBlocks} " +
                            "dropped=${attempt.droppedBlocks}"
                    )
                }
                ...
                generationDiagnostics = GenerationDiagnostics(
                    ...
                    contextBlocks = attempt.contextBlocks,
                    droppedBlocks = attempt.droppedBlocks,
                    retryUsed = attemptsUsed > 1,
                    attemptsUsed = attemptsUsed,
                    promptTokens = attempt.promptTokens,
                    generatedTokens = attempt.generatedTokens,
                    likelyTruncated = attempt.likelyTruncated,
                    sawChatmlStopToken = attempt.sawChatmlStopToken,
                    statsJson = attempt.statsJson,
                    error = attempt.error
                )
            }
            ...
            Log.i(
                ...
                "[${index + 1}/${items.size}] [${result.id}] type=${result.comparisonType} " +
                    ...
                    "sourceHit@3=%.2f allHit@3=%s mrr=%.2f/worst=%.2f wrongAdm@3=%s semSim=%s".format(
        ...
        val meanWrongAdmissionRateAt3 = scoredWrongAdmAt3.takeIf { it.isNotEmpty() }?.average()
        val scoredWrongAdmAt10 = results.mapNotNull { it.wrongAdmissionRateAt10 }
        val meanWrongAdmissionRateAt10 = scoredWrongAdmAt10.takeIf { it.isNotEmpty() }?.average()
        ...
        Log.i(
            TAG,
            "SUMMARY: n=$n source_hit@1=%.1f%% source_hit@3=%.1f%% source_hit@5=%.1f%% source_hit@10=%.1f%% ".format(
                meanSourceHitAt1 * 100, meanSourceHitAt3 * 100, meanSourceHitAt5 * 100, meanSourceHitAt10 * 100
            ) +
                "all_sources_hit@1=%.1f%% all_sources_hit@3=%.1f%% all_sources_hit@5=%.1f%% all_sources_hit@10=%.1f%% ".format(
                    allSourcesHitAt1Rate * 100, allSourcesHitAt3Rate * 100, allSourcesHitAt5Rate * 100, allSourcesHitAt10Rate * 100
                ) +
                ("mean_source_mrr=%.3f mean_worst_source_mrr=%.3f p@3=%.3f p@5=%.3f p@10=%.3f " +
                    "ndcg@3=%.3f ndcg@10=%.3f map=%.3f wrong_adm@3=%s wrong_adm@10=%s avg_sem_sim=%s pass_rate=%s").format(
                    ...
                    meanWrongAdmissionRateAt3?.let { "%.1f%%".format(it * 100) } ?: "n/a",
                    meanWrongAdmissionRateAt10?.let { "%.1f%%".format(it * 100) } ?: "n/a",
        ...
        for (type in results.map { it.comparisonType }.distinct()) {
            ...
            Log.i(
                ...
                "  [$type] n=$subN source_hit@3=%.1f%% worst_source_mrr=%.3f wrong_adm@3=%s pass_rate=%s".format(
                    ...
                    subWrongAdm3?.let { "%.1f%%".format(it * 100) } ?: "n/a",
        ...
        writeResultsJson(
            results, n, meanSourceHitAt1, meanSourceHitAt3, meanSourceHitAt5, meanSourceHitAt10,
    ...
    private fun writeResultsJson(
        ...
        val outFileName = "multidoc_rag_evaluation_results.json"
        ...
        root.put("generation_context_char_cap", GENERATION_CONTEXT_CHAR_CAP)
        root.put("generation_answer_token_reserve", 384)
        root.put("generation_blank_count", results.count { it.generatedAnswer?.isBlank() == true })
        root.put("generation_retry_count", results.count { it.generationDiagnostics?.retryUsed == true })
        root.put("generation_likely_truncated_count", results.count { it.generationDiagnostics?.likelyTruncated == true })
        root.put("generation_error_count", results.count { it.generationDiagnostics?.error != null })
        ...
        for (r in results) {
            ...
            itemJson.put("generation_stats_json", gd?.statsJson ?: JSONObject.NULL)
            itemJson.put("generation_error", gd?.error ?: JSONObject.NULL)
            itemJson.put("question", r.question)
            itemJson.put("top_chunks", JSONArray(r.topChunks))
