...
/** Retrieval AND generation evaluation for [HybridRagEngine] (semantic + lexical blend, tunable via `-e semanticWeight`), using the same dataset/pipeline/scoring as [MedicalRagEvaluationTest]. */
@RunWith(AndroidJUnit4::class)
class HybridRagEvaluationTest {

    companion object {
        private const val TAG = "HybridRagEvalTest"
        ...
        private const val TEST_DB_NAME = "objectbox-hybrid-rag-evaluation-test"
    ...
    private lateinit var vectorRepository: HybridVectorRepository
    private lateinit var embedder: Embedder
    private lateinit var pdfProcessor: ClinicalPdfProcessor
    private lateinit var ragEngine: HybridRagEngine
    ...
    fun setUp() {
        ...
        boxStore = MyObjectBox.builder()
            ...
            .name(TEST_DB_NAME)
            .build()
        val repo = HybridVectorRepository(boxStore)
        ...
        val semanticWeight = InstrumentationRegistry.getArguments()
            .getString("semanticWeight", "")
            ?.toDoubleOrNull() ?: 0.5
        ragEngine = HybridRagEngine(repo, embedder, semanticWeight)
        ...
        Log.i(TAG, "setUp: done (semanticWeight=$semanticWeight)")
    }
...
""".trimStart()
    ...
    fun runHybridRagEvaluation() {
        ...
        val ingestedDocuments = mutableSetOf<String>()
        for ((index, item) in items.withIndex()) {
            if (!ingestedDocuments.add(item.documentName)) {
                Log.i(TAG, "[${index + 1}/${items.size}] Skipped ${item.documentName}: already ingested")
                continue
            }
    ...
    private fun writeResultsJson(
        ...
        val outFileName = "hybrid_rag_results.json"
