...
/** Multi-document evaluation of [TfIdfRagEngine] (Table 3's "TF-IDF" row) against `QA/multidoc_dataset_llm_main_same_patient.jsonl`, the sparse-lexical counterpart to [MultiDocRagEvaluationTest] and [MultiDocBm25RagEvaluationTest]. */
@RunWith(AndroidJUnit4::class)
class MultiDocTfIdfRagEvaluationTest {

    companion object {
        private const val TAG = "MultiDocTfIdfRagEvalTest"
        ...
        private fun distinctiveTokens(text: String): Set<String> = tokenize(text).toSet()
    ...
    private lateinit var vectorRepository: TfIdfVectorRepository
    private lateinit var ragEngine: TfIdfRagEngine
    ...
    fun setUp() {
        ...
        val repo = TfIdfVectorRepository(targetContext)
        repo.clearAll()
        vectorRepository = repo
        ragEngine = TfIdfRagEngine(repo)
        Log.i(TAG, "setUp: constructing EmbeddingGemmaEmbedder (scoring only, not used for retrieval)")
        ...
        Log.i(TAG, "setUp: done (TfIdfRagEngine/TfIdfVectorRepository - pure JSON + TF-IDF, no embedder for retrieval)")
    }
...
""".trimStart()
    ...
    private fun packGenerationContexts(
        ...
        for (rank in 0 until GENERATION_CANDIDATE_TOP_K_PER_PATIENT) {
            for (pid in patientIds) {
                cappedByPatient[pid]?.getOrNull(rank)?.let { ordered.add(it) }
            }
        }
        ...
        for (block in ordered) {
            val fullPrompt = buildFactualQaPrompt(question, admitted + block)
    ...
    @Test
    fun runMultiDocTfIdfRagEvaluation() {
        val targetContext = InstrumentationRegistry.getInstrumentation().targetContext
        val testContext = InstrumentationRegistry.getInstrumentation().context
    ...
    private fun writeResultsJson(
        ...
        val outFileName = "multidoc_tfidf_rag_results.json"
