...
/** Retrieval AND generation evaluation for [Bm25RagEngine] (Table 3's "BM25" row), the one Table 3 test built to run correctly with a non-Qwen reader for Table 6's generator sweep. */
@RunWith(AndroidJUnit4::class)
class Bm25RagEvaluationTest {

    companion object {
        private const val TAG = "Bm25RagEvalTest"
    ...
    private lateinit var vectorRepository: Bm25VectorRepository
    private var embedder: EmbeddingGemmaEmbedder? = null
    private lateinit var pdfProcessor: ClinicalPdfProcessor
    private lateinit var ragEngine: Bm25RagEngine
    ...
    fun setUp() {
        ...
        val repo = Bm25VectorRepository(targetContext)
        repo.clearAll()
        ...
        if (!File(modelPath).exists() || !File(tokenizerPath).exists() ||
            ...
            Log.w(
                ...
                "Reader model not found at $modelPath / $tokenizerPath" +
                    ...
                    " - skipping generation evaluation (retrieval metrics are unaffected)."
            )
            return null
        }
    ...
    private fun buildFactualQaPrompt(question: String, contexts: List<String>): String {
        ...
        val basePrompt = "You are a factual question-answering assistant for clinical " +
...
$basePrompt<|eot_id|><|start_header_id|>user<|end_header_id|>
...
""".trimStart()
            "phi4" ->
                """
<|system|>$basePrompt<|end|><|user|>CLINICAL NOTES:
$contextBlock

QUESTION: $question<|end|><|assistant|>""".trimStart()
            "gemma3" ->
                """
<start_of_turn>user
$basePrompt
...
""".trimStart()
    ...
    fun runBm25RagEvaluation() {
        ...
        for ((index, item) in items.withIndex()) {
            ...
            val topChunks = retrieved.take(GENERATION_TOP_K).map { it.chunk.content }
    ...
    private fun writeResultsJson(
        ...
        val outFileName = "bm25_rag_results.json"
        ...
        for (r in results) {
            itemsJson.put(
                JSONObject().apply {
                    ...
                    put("top_chunks", JSONArray(r.topChunks))
                }
            )
        }
