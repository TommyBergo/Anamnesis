...
/**
 * Utility object for formatting prompts based on model type.
 */
object PromptFormat {
    ...
    fun getSystemPromptTemplate(modelType: ModelType): String {
        return when (modelType) {
            ModelType.LLAMA_3 -> "<|begin_of_text|><|start_header_id|>system<|end_header_id|>\n" +
                    SYSTEM_PLACEHOLDER +
                    "<|eot_id|>"
            ModelType.QWEN_3, ModelType.SMOLLM_3, ModelType.LFM2 ->
                "<|im_start|>system\n$SYSTEM_PLACEHOLDER<|im_end|>\n"
            ModelType.PHI_4 -> "<|system|>$SYSTEM_PLACEHOLDER<|end|>"
    ...
    fun getUserPromptTemplate(modelType: ModelType): String {
        return when (modelType) {
            ...
            ModelType.PHI_4 ->
                "<|user|>" + USER_PLACEHOLDER + "<|end|><|assistant|>"
    ...
    fun getStopToken(modelType: ModelType): String {
        return when (modelType) {
            ...
            ModelType.QWEN_3, ModelType.SMOLLM_3, ModelType.LFM2 -> "<|im_end|>"
            ModelType.VOXTRAL -> "</s>"
            ModelType.PHI_4 -> "<|end|>"
        }
    }
    ...
    fun replaceSpecialToken(modelType: ModelType, token: String): String {
        return when (modelType) {
            ModelType.QWEN_3, ModelType.SMOLLM_3, ModelType.LFM2 -> when (token) {
                "<|im_end|>" -> ""
                else -> token
            }
            else -> token
